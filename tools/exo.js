/* O CENSO DOS EXOPLANETAS (r310): baixa a tabela pscomppars do NASA Exoplanet
   ...
   Formato gravado (window.EXO):
     ...
     h[i]  = [nome, RA em horas, Dec em graus, distancia em anos-luz, n planetas, temperado 0/1, tipo espectral]
...
const COLS = 'hostname,ra,dec,sy_dist,pl_name,pl_rade,pl_bmasse,pl_orbper,pl_eqt,disc_year,discoverymethod,st_spectype';
...
(async ()=>{
  ...
  for(const c of rows){
    const [host, ra, dec, dist, pl, rade, masse, per, teq, ano, metodo, spec] = c;
    const dpc = parseFloat(dist); if(!isFinite(dpc) || dpc <= 0) continue;
    if(!hosts.has(host)) hosts.set(host, {i:hosts.size, nome:host, ra:num(+ra/15, 3), dec:num(dec, 2), ly:num(dpc*3.26156, 1), n:0, temp:0, spec:(spec || '').trim()});
  ...
  const h = [...hosts.values()].map(H => [H.nome, H.ra, H.dec, H.ly, H.n, H.temp, H.spec]);
