...
for(const f of ['en.js', 'dados_en.js', 'ceu6.js', 'tle_snapshot.js', 'exo.js'].concat(fs.existsSync(path.join(__dirname, '..', 'efem')) ? fs.readdirSync(path.join(__dirname, '..', 'efem')).filter(x=>/\.js$/.test(x)).map(x=>'efem/' + x) : [])){   /* r316 · as rotas reais também compilam */
